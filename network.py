...
class MelDecoder(nn.Module):
    ...
    def __init__(self, num_hidden):
        ...
        self.decoder_prenet = Prenet(
            hp.num_mels, num_hidden * 2, num_hidden, p=0.2)
    ...
    def forward(self, memory, decoder_input, c_mask, pos):
        ...
        if self.training:
            ...
            mask = mask + torch.triu(torch.ones(decoder_len, decoder_len).cuda(),
            ...
            mask = torch.triu(torch.ones(decoder_len, decoder_len).cuda(),
                              diagonal=1).repeat(batch_size, 1, 1).byte()
            mask = mask.gt(0)
        ...
        stop_tokens = stop_tokens.squeeze()
